...
class OpenMatchTrainerProbe:
        ...
        if len(unlabel_gt) != 0:
            for gt, pseudo in zip(unlabel_gt, unlabel_pseudo):
                ...
                if len(gt) != 0 and len(pseudo) != 0:
                    max_iou, max_idx = pairwise_iou(gt.gt_boxes.to('cuda'), pp_boxes).max(1)
    ...
    def compute_num_box(self, unlabel_gt, unlabel_pseudo, name, processed=False):
            ...
            for gt, pp_roi in zip(unlabel_gt, unlabel_pseudo):
                ...
                if name == "gt":
                    ...
                    if len(gt.gt_boxes.to('cuda')) > 0 and len(pp_boxes) > 0:
                        # gt.gt_boxes: Box(tensor with shape (#gt_bbox, 4))
                        # gt.gt_classes: (#gt_bbox,)
                        # pp_boxes: Box(tensor with shape (#pseudo_bbox, 4))
                        
                        pairwise_ious = pairwise_iou(gt.gt_boxes.to('cuda'), pp_boxes)  # (#gt_bbox, #pseudo_bbox)
                        max_iou, max_idx = pairwise_ious.max(0)  # (#pseudo_bbox,), (#pseudo_bbox,)
    
                        # for the ground-truth label for each pseudo-box
                        gtclass4pseudo = gt.gt_classes[max_idx]  # cat label for each pseudo-box
                        matchgtbox = max_iou > 0.5  # True means that a pseudo-box is foreground
    
                        # compute the number of boxes (background, inlier, outlier)
                        num_bg += (~matchgtbox).sum().item()
                        num_bbox_in += (gtclass4pseudo[matchgtbox] != -1).sum().item()
                        num_bbox_out += (gtclass4pseudo[matchgtbox] == -1).sum().item()
                        num_valid_img += 1
        ...
        if num_bbox == 0:
            return box_probe, []
        if num_valid_img > 0 :
            box_probe["Analysis_" + name + "/Num_bbox"] = num_bbox / num_valid_img
            box_probe["Analysis_" + name + "/Size_bbox"] = size_bbox / num_valid_img
            box_probe["Analysis_" + name + "/Num_bbox_inlier"] = num_bbox_in / num_valid_img
            box_probe["Analysis_" + name + "/Num_bbox_outlier"] = num_bbox_out / num_valid_img
    
            if name != "gt":  # prediciton, background number
                box_probe["Analysis_" + name + "/Conf"] = avg_conf / num_valid_img
                box_probe["Analysis_" + name + "/Num_bbox_background"] = num_bg / num_valid_img
                box_probe["Analysis_" + name + "/background_fp_ratio"] = num_bg / num_bbox
                box_probe["Analysis_" + name + "/background_tp_ratio"] = num_bbox_in / num_bbox
        else:
            box_probe["Analysis_" + name + "/Num_bbox"] = 0.0
            box_probe["Analysis_" + name + "/Size_bbox"] = 0.0
            box_probe["Analysis_" + name + "/Num_bbox_inlier"] = 0.0
            box_probe["Analysis_" + name + "/Num_bbox_outlier"] = 0.0
    
            if name != "gt":  # prediciton, background number
                box_probe["Analysis_" + name + "/Conf"] = 0.0
                box_probe["Analysis_" + name + "/Num_bbox_background"] = 0.0
                box_probe["Analysis_" + name + "/background_fp_ratio"] = num_bg / num_bbox
                box_probe["Analysis_" + name + "/background_tp_ratio"] = num_bbox_in / num_bbox

...
class DAobjTwoStagePseudoLabGeneralizedRCNN(GeneralizedRCNN):
    ...
    def forward(
        ...
        """
        # if self.D_img == None:  # False
        #     self.build_discriminator()

        if (not self.training) and (not val_mode):  # only conduct when testing mode
            if not self.pseudo_bbox_mode:
                if isinstance(batched_inputs[0], tuple):  # generate pseudo labels by the inference method
                    batched_inputs = [weak_strong_data_pair[1] for weak_strong_data_pair in batched_inputs]  # extract unlabel_data_k
                
                return self.inference(batched_inputs, do_postprocess=self.do_postprocess)
            else:
                batched_inputs = [weak_strong_data_pair[1] for weak_strong_data_pair in batched_inputs]  # extract unlabel_data_k
                branch = 'unsup_data_weak'  # get the output proposals of rpn and roi-head
        ...
        if branch == "domain":
            ...
            domain_labels = torch.FloatTensor(D_img_out_s.data.size()).fill_(source_label).to(self.device)
            # if 'domain_label' in batched_inputs[0]:
            #     for i, batched_input in enumerate(batched_inputs):
            #         domain_labels[i] = batched_input['domain_label']
            for i, batched_input in enumerate(batched_inputs):
                if 'mosaic_mask' in batched_input:
                    mask = batched_input['mosaic_mask']
                    tgt_size = domain_labels.shape[2:]
                    domain_labels[i] = f.resize(mask.unsqueeze(0), tgt_size)  # BILINEAR
                elif 'domain_label' in batched_input:
                    domain_labels[i] = batched_input['domain_label']

            loss_D_img_s = F.binary_cross_entropy_with_logits(D_img_out_s, domain_labels)
        ...
        if 'res4' in features:
            if features['res4'] is not None:
                if torch.isinf(features['res4']).any():
                    print('features has inf:', torch.isinf(features['res4']).any())
                    breakpoint()
        ...
        if branch == "supervised":
            ...
            if self.with_grl:
                ...
                ### D_img_out_s.data.size(): (bs, 1, 18, 37)
                loss_D_img_s = F.binary_cross_entropy_with_logits(D_img_out_s, domain_labels)
            ...
            return losses, [], proposals_roih, None, features[self.dis_type]

        elif branch == "supervised_wo_grl":
            proposals_rpn, proposal_losses = self.proposal_generator(
                images, features, gt_instances
            )
        ...
        elif branch == "extract_backbone_features":
            return features[self.dis_type]

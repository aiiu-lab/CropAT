...
def build_clip_resnet_backbone(cfg, input_shape):
    ...
    resnet = ModifiedResNet(layers=vision_layers, 
    ...
    ckpt = torch.load(cfg.MODEL.REGIONCLIP_WEIGHTS)
    ckpt = {k.replace('backbone.', ''): v for k, v in ckpt['model'].items() if k.startswith('backbone')}
    resnet.load_state_dict(ckpt, strict=False)
    print('Loaded RegionCLIP ResNet50 weights')

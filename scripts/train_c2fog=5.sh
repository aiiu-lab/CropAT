CUDA_VISIBLE_DEVICES=0,1,2,3 python train_net.py \
    ...
    OUTPUT_DIR exps/c2fog=5 \
    DATASETS.TRAIN_UNLABEL "('cityscapes_c_fog=5_train',)" \
    DATASETS.TEST "('cityscapes_fine_instance_seg_val','cityscapes_c_fog=5_val')" \
    DATASETS.TRAIN_TARGET_LIKE_LABEL "('target_like_cityscapes_c_fog=5_train',)" \
    ...
    DATASETS.ADD_CROPS.SRC.TARGET_LIKE_CROP_IMG_DIR "datasets/target_like/c2fog=5/leftImg8bit/crops" \
    DATASETS.ADD_CROPS.SRC.NUM_CROPS 5 \
    DATASETS.ADD_CROPS.TGT.CROP_IMG_DIR "datasets/target_like/c2fog=5/leftImg8bit/crops" \
